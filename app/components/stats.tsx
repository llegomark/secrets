...
export const Stats = asyncComponent(async () => {
  const [reads, writes] = await redis
    ...
    .exec<[number, number]>();
  const stars = await fetch("https://api.github.com/repos/llegomark/secrets")
    .then((res) => res.json())
  ...
  return (
    <section className="container mx-auto">
      <ul className="grid grid-cols-1 gap-x-2 sm:gap-x-4 sm:grid-cols-3">
        {stats.map(({ label, value }) => (
          <li
            key={label}
            className="flex items-center justify-center gap-2 px-4 py-3 overflow-hidden rounded m sm:flex-col"
          >
